...
module.exports = class SoftBanCommand extends Command {
  ...
  async run(message, args) {
    ...
    if (member === message.member) 
      return this.sendErrorMessage(message, 'Invalid argument. You cannot softban yourself.'); 
    if (member.roles.highest.position >= message.member.roles.highest.position)
      return this.sendErrorMessage(message, `
        Invalid argument. You cannot softban someone with an equal or higher role.
      `);
    ...
    await message.guild.unban(member.user, reason);
    ...
    message.client.logger.info(`${message.guild.name}: ${message.member.displayName} softbanned ${member.displayName}`);
