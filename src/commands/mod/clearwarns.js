...
module.exports = class ClearWarnsCommand extends Command {
  ...
  run(message, args) {
    ...
    if (member === message.member) 
      return this.sendErrorMessage(message, 'Invalid argument. You cannot clear your own warns.'); 
    if (member.roles.highest.position >= message.member.roles.highest.position)
      return this.sendErrorMessage(message, `
        Invalid argument. You cannot clear the warns of someone with an equal or higher role.
      `);
