module.exports = {
  ...
  run: async (message, args) => {
    ...
    //args check
    if (!color) return message.channel.send(`Sorry ${message.member}, I don't recognize that color. Type \`!colors\` for a list.`);
    else if (message.member.roles.has(color.id)) return message.channel.send(`${message.member}, you are already ${color}!`);
    if (message.guild.me.hasPermission('MANAGE_ROLES')){
      try {
        ...
        message.channel.send(`Sorry ${message.member}, something went wrong. Please confirm that I am at the top of the role hierarchy.`);
      }
    }
